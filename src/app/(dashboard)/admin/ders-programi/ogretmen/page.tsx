...
export default function OgretmenDersProgramiPage() {
  ...
  const {
    ...
    userId: 'admin-1',
    type: 'ders-programi',
    ogretmenId: viewingSchedule || undefined,
  })

  const handleViewSchedule = (ogretmenId: string) => {
    setViewingSchedule(ogretmenId)
  }
  ...
  const handleDirectDownload = async (ogretmenId: string) => {
    try {
      setDownloadingIds(prev => new Set(prev).add(ogretmenId))

      const doc = await pdfViewerService.getPdfByTeacher(ogretmenId, 'ders-programi')
                  ...
                  <tr key={schedule.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-4">
                      <div className="flex items-center justify-center gap-2">
                        <button
                          onClick={() => handleViewSchedule(schedule.ogretmenId)}
                        ...
                        <button
                          onClick={() => handleDirectDownload(schedule.ogretmenId)}
                          disabled={downloadingIds.has(schedule.ogretmenId)}
                          className="rounded-lg p-2 text-green-600 hover:bg-green-50 disabled:opacity-50"
                          title="İndir"
                        >
                          {downloadingIds.has(schedule.ogretmenId) ? (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="relative w-full max-w-6xl max-h-[90vh] overflow-auto bg-white rounded-lg shadow-xl">
            <div className="sticky top-0 z-10 flex items-center justify-between border-b bg-white px-6 py-4">
              <h2 className="text-xl font-bold">Öğretmen Ders Programı Görüntüleme</h2>
              <button
